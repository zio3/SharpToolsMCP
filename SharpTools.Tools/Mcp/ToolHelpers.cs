...
internal static class ToolHelpers {
    ...
    public static void EnsureSolutionLoaded(ISolutionManager solutionManager) {
        if (!solutionManager.IsSolutionLoaded) {
            throw new McpException($"No solution is currently loaded. SharpTools now operates in stateless mode - provide the necessary context path instead.");
        }
    }
    ...
    public static void EnsureSolutionLoadedWithDetails(ISolutionManager solutionManager, ILogger logger, string operationName) {
        if (!solutionManager.IsSolutionLoaded) {
            ...
            throw new McpException($"No solution is currently loaded. SharpTools now operates in stateless mode - use the stateless version of '{operationName}' with the necessary context path.");
        }
    }
    private const string FqnHelpMessage = $" Try `{ToolHelpers.SharpToolPrefix}{nameof(Tools.AnalysisTools.GetMembers)}` or `{ToolHelpers.SharpToolPrefix}{nameof(Tools.DocumentTools.ReadTypesFromRoslynDocument)}`  to find what you need.";
